<template>
  <div class="dictTable">
    <section class="dictTableSection" @click = "sortVisiable = false">
          ...
          <el-col :span="17" style="float:left;">
            <el-row style="float:right;">
              ...
              <span style="display: inline-block; float: right;">
                <el-col :span="9" style="float:left; margin-left:10px;height:36px;">
                  <el-select v-model="selectionModule" placeholder="模块选择" @change="handleModuleChange">
                    <el-option
                ...
                <el-col :span="13" style="float:left;">
                  <ul class="rightUl">
            				<li class="sort">
                      <span class="sortable" @click.stop="sortVisiableFun">排序</span>
                      <ul class="sortOptions" v-show="sortVisiable">
                        <li>
                          <span>字典代码</span>
                          <em >
                            <span class="up" :class="codeOrderOption==='code'? 'upSelect': ''" @click="orderFun('code', '','dictCode')"></span>
                            <span class="down" :class="codeOrderOption==='code-'? 'downSelect': ''"  @click="orderFun('code', '-','dictCode')"></span>
                          </em>
                          </li>
                          <li>
                            <span>选项值</span>
                            <em >
                              <span class="up" :class="valueOrderOption==='value'? 'upSelect': ''"  @click="orderFun('value', '','optionValue')"  ></span>
                              <span class="down"  :class="valueOrderOption==='value-'? 'downSelect': ''" @click="orderFun('value', '-','optionValue')" ></span>
                            </em>
                          </li>
                      </ul>
            				</li>
            				...
            				<li>
            					<span @click="create" v-if="superAdmin">新建</span>
            				</li>
                    <li>
                      <span @click="sysDeleteArry" v-if="superAdmin">删除</span>
                    </li>
            			</ul>
                </el-col>
              </span>
      ...
      <el-col :span="24" style="background: #eee;padding: 15px;">
        <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark"  v-loading="listLoading" @selection-change="selsChange" style="width: 100%;" :stripe="true">
    			<el-table-column type="selection" width="55" v-if="superAdmin">
    			</el-table-column>
    			<el-table-column type="index" width="80" label="序号">
    			</el-table-column>

    			<el-table-column prop="dictCode" min-width="250" label="字典代码" show-overflow-tooltip>
    			</el-table-column>
    			<el-table-column prop="optionValue" label="字典值" min-width="140" show-overflow-tooltip>
    			</el-table-column>
    			<el-table-column label="字典名称" min-width="200" >
            <template scope="scope">
              <span :title="scope.row.optionName" style="display: inline-block;float: left;width:120px;overflow: hidden;text-overflow: ellipsis;white-space:nowrap;">{{scope.row.optionName}}</span>
              <span style="display: inline-block;float: left;line-height: 22px;" v-if="scope.row.sourceId" :title="`原：${scope.row.sourceTitle}`"><el-tag  type="success">副本</el-tag></span>
            </template>
    			</el-table-column>
          <el-table-column prop="proName" label="模块" min-width="140" :formatter="formatter">

    			</el-table-column>
    			<el-table-column prop="orderNo" label="排序" min-width="80">
    			</el-table-column>
    			<el-table-column label="操作" min-width="120">
    				<template scope="scope">
    					<i class="create" title="编辑" v-if="superAdmin" @click="editDict(scope.$index, scope.row)"></i>
    					<i class="edit" title="编辑" v-if="institutesAdmin && !superAdmin" @click="createDict(scope.$index, scope.row)"></i>
              <i class="watch" title="查看" @click="watchDict(scope.$index, scope.row)"></i>
              <i class="delete" title="删除" v-if="superAdmin" @click="sysDeleteDict(scope.$index, scope.row)"></i>
              <i class="delete" title="删除" v-if="institutesAdmin && scope.row.sourceId" @click="classDeleteDict(scope.$index, scope.row)"></i>
    				</template>
    			</el-table-column>
    		</el-table>
      </el-col>
      <el-col :span="24" style="background: #eee;padding: 0px 15px;">
        <el-row>
          <el-col :span="24" class="footbar">
            <el-checkbox v-model="checked" v-on:change="toogleSelection()"  v-if="superAdmin"></el-checkbox>
            <el-button type="danger" @click="sysDeleteArry" :disabled="this.sels.length===0"  v-if="superAdmin">批量删除</el-button>
